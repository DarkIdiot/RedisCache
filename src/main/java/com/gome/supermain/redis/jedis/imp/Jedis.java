package com.gome.supermain.redis.jedis.imp;

import static com.gome.supermain.redis.common.JedisType.READ;
import static com.gome.supermain.redis.common.JedisType.WRITE;
...
import com.gome.supermain.redis.common.JedisType;
...
public class Jedis implements IJedis {
	...
	public Jedis(JedisPool writeJedisPool, JedisPool readJedisPool) {
	...
	public List<? extends Object> hmget(final Object key, final List<? extends Object> fields) {
		byte[] keyBytes = ByteObjectConvertUtil.getBytesFromObject(key);
		byte[][] fieldBytes = new byte[fields.size()][];
		for (int i = 0; i < fields.size(); i++) {
			if (fields.get(i) != null) {
				fieldBytes[i] = ByteObjectConvertUtil.getBytesFromObject(fields.get(i));
			}
		}
		List<byte[]> retBytes = hmget(keyBytes, fieldBytes);
		if (retBytes != null && retBytes.size() > 0) {
			List<Object> rets = new ArrayList<Object>();
			for (byte[] valueByte : retBytes) {
				if (valueByte != null && valueByte.length > 0) {
					rets.add(ByteObjectConvertUtil.getObjectFromBytes(valueByte));
				}
			}
			return rets;
		}
